...
const SimpleStudentView = () => {
  ...
  useEffect(() => {
    const loadVideos = async () => {
      ...
      try {
        ...
        const videosData = await simpleApi.getVideos();
  ...
  const handleToggleVideo = async (videoId: string) => {
    ...
    try {
      ...
      const success = await updateProgress(videoId, newCompleted);
      if (success) {
  ...
  const completedCount = user.completedVideos.length;
...
export default SimpleStudentView;
